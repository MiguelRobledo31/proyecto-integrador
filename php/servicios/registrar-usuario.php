...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($usuario && $password && $nombre && $apellidos && $rol) {
    $stmt = $conexion->prepare("SELECT id FROM usuarios WHERE usuario = ? AND rol = ?");
    $stmt->bind_param("ss", $usuario, $rol);
    $stmt->execute();
    $stmt->store_result();

    if ($stmt->num_rows > 0) {
      echo "<script>alert('Ya existe un usuario con ese usuario o rol'); window.location.href = 'registrar-usuario.php';</script>";
      exit;
    } else {
      $stmt->close();
      $stmt = $conexion->prepare("INSERT INTO usuarios (usuario, password, nombre, apellidos, rol) VALUES (?, ?, ?, ?, ?)");
      $stmt->bind_param("sssss", $usuario, $password, $nombre, $apellidos, $rol);
      $stmt->execute();
...
<body>
  ...
  <main class="main-content">
    ...
    <section class="contenido">
      ...
      <?php if (!empty($alerta)): ?>
        <div class="alerta-error" style="color: red; font-weight: bold; margin-bottom: 10px;">
          <i class="bi bi-exclamation-circle-fill" style="margin-right: 5px;"></i> <?= $alerta ?>
        </div>
      <?php endif; ?>
